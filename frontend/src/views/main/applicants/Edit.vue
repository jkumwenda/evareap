<template>
  <div class="flex flex-col space-y-4 flex-1">
    ...
    <div v-else class="rounded-2xl bg-catskill-white-100 shadow-sm p-4">
      <form class="flex flex-col w-5/12 space-y-4" @submit.prevent="editApplicant" method="POST">
        <label class="block">
...
export default {
  name: "EditApplicantView",
  components: {
    HeaderView, SpinnerComponent
  },
  data() {
    return {
      headerTitle: "Edit applicant",
  ...
  methods: {
    ...
    async getApplicant() {
      try {
        ...
        this.applicantData.applicant = response.applicant.applicant;
        this.applicantData.address = response.applicant.address;
        this.applicantData.country_id = response.applicant.country_id;
        this.applicantData.email = response.applicant.email;
        this.applicantData.phone = response.applicant.phone;
        this.isLoading = false;
      } catch (error) {
