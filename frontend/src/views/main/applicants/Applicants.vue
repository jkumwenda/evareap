    ...
    <div class="flex flex-col space-y-4 flex-1">
        ...
        <div class="flex flex-col space-y-4">
            <div class="flex justify-between items-center">
                ...
                <router-link :to="{ name: 'AddApplicant' }" v-if="permissions.includes('ADD_APPLICANT')"
            ...
            <div v-else class="rounded-2xl bg-catskill-white-100 shadow-sm p-4">
                ...
                <div class="flex flex-row p-2 text-sm items-center" v-for="(applicant, index) in applicants"
                    :key="applicant.id" :class="getRowClass(index)">
                    <div class="sm:w-3/12">{{ applicant.applicant }}</div>
                    <div class="sm:w-3/12">{{ applicant.address }}</div>
                    <div class="sm:w-2/12">{{ applicant.country.country }}</div>
                    <div class="sm:w-2/12">{{ applicant.phone }}</div>
                    <div class="sm:w-2/12">{{ applicant.email }}</div>
                    <div class="flex space-x-2 sm:w-1/12">
                        <router-link v-if="permissions.includes('VIEW_APPLICANT')"
                            class="p-1 bg-dodger-blue-50 rounded-full"
                            :to="{ name: 'Applicant', params: { id: applicant.id } }">
                            <EyeIcon class="w-5 h-5 text-catalina-blue-300"></EyeIcon>
                        </router-link>
                        <router-link v-if="permissions.includes('UPDATE_APPLICANT')" class="p-1 bg-spray-200 rounded-full"
                            :to="{ name: 'EditApplicant', params: { id: applicant.id } }">
                            <PencilSquareIcon class=" w-5 h-5 text-spray-700">
                            </PencilSquareIcon>
                        </router-link>
                        <div v-if="permissions.includes('DELETE_APPLICANT')" class="p-1 bg-flamingo-100 rounded-full">
                            <TrashIcon class="w-5 h-5 text-flamingo-500" @click="showDeleteConfirmation(applicant.id)">
                            </TrashIcon>
                        </div>
                    </div>
                </div>
        ...
        <delete-confirmation-modal :show="showDeleteModal" @confirmed="deleteApplicant(deleteApplicantId)"
...
export default {
    ...
    methods: {
        ...
        async deleteApplicant(id) {
            this.isLoading = true;
            try {
                await deleteItem("applicants", id);
