<template>
  <div class="flex flex-col space-y-4 flex-1">
    ...
    <div v-else class="rounded-2xl bg-catskill-white-100 shadow-sm p-4">
      <form class="flex flex-col w-5/12 space-y-4" @submit.prevent="addGenericName" method="POST">
        <label class="block">
...
export default {
  name: "AddGenericNameView",
  components: {
    HeaderView, SpinnerComponent
  },
  data() {
    return {
      headerTitle: "Add generic name",
      genericNameData: {
        generic_name: "",
        description: "",
      },
  ...
  methods: {
    async addGenericName() {
      this.isLoading = true;
      try {
        const response = await createItem("generic_names/", this.genericNameData);
