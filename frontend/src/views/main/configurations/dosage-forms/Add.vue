<template>
  <div class="flex flex-col space-y-4 flex-1">
    ...
    <div v-else class="rounded-2xl bg-catskill-white-100 shadow-sm p-4">
      <form class="flex flex-col w-5/12 space-y-4" @submit.prevent="addDosageForm" method="POST">
        <label class="block">
...
export default {
  name: "AddDosageFormView",
  components: {
    HeaderView, SpinnerComponent
  },
  data() {
    return {
      headerTitle: "Add dosage form",
      dosageFormData: {
        dosage_form: "",
        description: "",
      },
  ...
  methods: {
    async addDosageForm() {
      this.isLoading = true;
      try {
        const response = await createItem("dosage_forms/", this.dosageFormData);
